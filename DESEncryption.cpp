...
void encrypt(string pt, string k, int n) {
	...
	for (int i = 0; i < n; i++) {
		ip = permutate(pt, InitialPermutation);
		L = ip.substr(0, 32);
		R = ip.substr(32, 32);
...
int main()
{
	...
	cin >> key >> plainText >> nofrounds;


	encrypt(plainText, key, nofrounds);
